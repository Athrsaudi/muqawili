...
export default function FileUploader({ bucket, folder, existingFiles = [], onFilesChange, maxFiles = 10, label = 'المرفقات' }) {
  ...
  async function removeFile(idx) {
    ...
    if (file.url) {
      const pathPart = file.url.split('/storage/v1/object/public/' + bucket + '/')[1];
      if (pathPart) await supabase.storage.from(bucket).remove([pathPart]);
    }
      ...
      {files.length > 0 && (
        <div className="file-list">
          {files.map((file, i) => {
            ...
            const name = file.name || decodeURIComponent(url.split('/').pop().split('?')[0]);
            const isImage = /\.(jpg|jpeg|png|gif|webp)$/i.test(url) || String(file.type).startsWith('image');
            ...
            return (
              <div key={i} className="file-item">
                {isImage
                  ...
                  : <div className="file-preview-pdf"><span>{isVideo ? '🎬' : isPDF ? '📄' : '📎'}</span></div>
                }
