...
async function upload(sel){
  ...
  const next=[...files,...added];setFiles(next);onFilesChange(next.map(x=>x.url));
...
<div className="file-uploader">
  ...
  {files.length>0&&<div className="file-list">{files.map((f,i)=>{
    const url=f.url||f;const name=f.name||url.split('/').pop();
    ...
    return(<div key={i} className="file-item">
      {isImg?<img src={url} alt={name} className="file-preview-img"/>:<div className="file-preview-pdf"><span>{isVid?'🎬':isPdf?'📄':'📎'}</span></div>}
      <div className="file-item-info"><a href={url} target="_blank" rel="noopener noreferrer" className="file-item-name">{name}</a>{f.size&&<span className="file-item-size">{fmt(f.size)}</span>}</div>
