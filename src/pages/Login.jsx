...
export default function Login() {
  ...
  async function handleLogin() {
    ...
    // Use national_id + phone as unique email to avoid email sending
    ...
    if (e1) {
      // Try creating auth record
      const { error: e2 } = await supabase.auth.signUp({ email: authEmail, password: nationalId, options: { emailRedirectTo: undefined, data: { skip_confirmation: true } } })
      if (!e2) {
        // Update email_confirmed_at directly
        await supabase.rpc('confirm_user_email', { user_email: authEmail })
        const { error: e3 } = await supabase.auth.signInWithPassword({ email: authEmail, password: nationalId })
        if (e3) { setError('حدث خطأ في تسجيل الدخول'); setLoading(false); return }
      } else { setError('حدث خطأ في تسجيل الدخول'); setLoading(false); return }
    }
  ...
  async function handleRegister() {
    ...
    // Use national_id + phone as unique fake email - no email sent
    const authEmail = regNationalId + fp.replace('+','') + '@mq.sa'
    const { data: auth, error: e1 } = await supabase.auth.signUp({ email: authEmail, password: regNationalId })
    if (e1 || !auth.user) { setError('حدث خطأ: ' + (e1?.message || '')); setLoading(false); return }
    const { error: e2 } = await supabase.from('users').insert({ id: auth.user.id, phone: fp, email: email.trim() || null, full_name: fullName.trim(), national_id: regNationalId, user_type: userType, city, is_verified: true })
