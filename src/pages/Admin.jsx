...
export default function AdminPanel() {
 ...
 <td><span className={'admin-type-badge ' + u.user_type}>{u.user_type === 'contractor' ? 'مقاول' : u.user_type === 'client' ? 'عميل' : 'مدير'}</span></td>
 <td>{u.city}</td>
 ...
 <td><span className={'admin-status-dot ' + (u.is_active ? 'active' : 'inactive')}>{u.is_active ? 'نشط' : 'موقوف'}</span></td>
 <td><button className={'admin-toggle-btn ' + (u.is_active ? 'deactivate' : 'activate')} onClick={() => toggleActive(u.id, u.is_active)} disabled={actionLoading === u.id + '-active'}>{u.is_active ? 'إيقاف' : 'تفعيل'}</button></td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div className="admin-table-container">
 <table className="admin-table">
 <thead><tr><th>العنوان</th><th>العميل</th><th>التصنيف</th><th>المدينة</th><th>الحالة</th><th>التاريخ</th><th>إجراء</th></tr></thead>
 <tbody>
 {requests.map(r => (
 <tr key={r.id}>
 <td className="admin-td-name"><a href={'/requests/' + r.id} className="admin-req-link">{r.title}</a></td>
 <td>{r.users?.full_name}</td>
 <td><span className="admin-cat-badge">{CAT_LABELS[r.category] || r.category}</span></td>
 <td>{r.city}</td>
 <td><span className={'admin-req-status ' + r.status}>{r.status === 'open' ? 'مفتوح' : r.status === 'in_progress' ? 'جارٍ' : r.status === 'closed' ? 'مغلق' : 'ملغي'}</span></td>
 <td className="admin-td-date">{new Date(r.created_at).toLocaleDateString('ar-SA')}</td>
 <td className="admin-td-actions">
 {r.status === 'open' && <button className="admin-close-btn" onClick={() => closeRequest(r.id)} disabled={actionLoading === r.id + '-close'}>إغلاق</button>}
 <button className="admin-delete-btn" onClick={() => deleteRequest(r.id)} disabled={actionLoading === r.id + '-delete'}>حذف</button>
 </td>
 </tr>
 ))}
 </tbody>
