...
const CITIES = [
  ...
  'الدمام','الخبر','تبوك','أبها','حائل','القصيم','نجران','جازان'
]
...
export default function Login() {
  ...
  async function handleLogin() {
    ...
    if (!nationalId.match(/^[12]\d{9}$/)) { setError('رقم الهوية يجب أن يبدأ بـ 1 أو 2 ويكون 10 أرقام'); setLoading(false); return }
    ...
    if (signInErr) {
      ...
      const { error: retryErr } = await supabase.auth.signInWithPassword({ email: fakeEmail, password: nationalId })
  ...
  async function handleRegister() {
    ...
    if (fp.length < 13) { setError('أدخل رقم جوال سعودي صحيح'); setLoading(false); return }
    if (!regNationalId.match(/^[12]\d{9}$/)) { setError('رقم الهوية يجب أن يبدأ بـ 1 أو 2 ويكون 10 أرقام'); setLoading(false); return }
    if (!city) { setError('اختر مدينتك'); setLoading(false); return }

    const { data: existing } = await supabase.from('users').select('id')
      .or(`phone.eq.${fp},national_id.eq.${regNationalId}`).maybeSingle()
    ...
    const { error: insertErr } = await supabase.from('users').insert({
      id: authData.user.id, phone: fp, full_name: fullName.trim(),
      national_id: regNationalId, user_type: userType, city, is_verified: true,
    })

    if (insertErr) { setError(insertErr.code === '23505' ? 'هذه الهوية مسجلة مسبقاً' : 'حدث خطأ، حاول مرة أخرى'); setLoading(false); return }
          ...
          {tab === 'login' && (
            <div className="auth-form">
              <div className="field">
                ...
                <input type="tel" placeholder="05xxxxxxxx" value={phone} onChange={e => setPhone(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} />
              </div>
              <div className="field">
                <label>رقم الهوية الوطنية</label>
                <input type="text" placeholder="1xxxxxxxxx" value={nationalId} onChange={e => setNationalId(e.target.value)} maxLength={10} onKeyDown={e => e.key === 'Enter' && handleLogin()} />
              </div>
          ...
          {tab === 'register' && (
            <div className="auth-form">
              <div className="field">
                ...
                <input type="text" placeholder="محمد بن أحمد" value={fullName} onChange={e => setFullName(e.target.value)} />
              </div>
              <div className="field">
                <label>رقم الجوال</label>
                <input type="tel" placeholder="05xxxxxxxx" value={regPhone} onChange={e => setRegPhone(e.target.value)} />
              </div>
              <div className="field">
                <label>رقم الهوية الوطنية</label>
                <input type="text" placeholder="1xxxxxxxxx" value={regNationalId} onChange={e => setRegNationalId(e.target.value)} maxLength={10} />
              </div>
              <div className="field">
                <label>المدينة</label>
